...
```powershell
cd ~\Downloads\fps-boost-main
.\FPS-Boost.ps1
```

**4.** Pick an option from the menu and **reboot** after applying.
...
A companion script that **automatically disables unnecessary startup apps** (OneDrive, updaters, tray tools, ...) while **protecting the ones you actually want at boot** — GPU drivers, audio, mouse/keyboard software (Logitech, Razer, SteelSeries, ...) and your antivirus.
...
```
fps-boost/
├── FPS-Boost.ps1        # main script
├── Autostart-Clean.ps1  # bonus autostart cleaner
├── screenshots/
├── LICENSE
└── README.md
```
